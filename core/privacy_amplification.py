...
def toeplitz_hash(
    ...
    Constructs a random Toeplitz matrix T ∈ {0,1}^(output_length × input_length)
    and computes the compressed key: k' = T × k (mod 2).

    A Toeplitz matrix is defined by its first row and first column,
    requiring only (n + m - 1) random bits instead of n × m.

    Security guarantee (leftover hash lemma):
        If H_min(K|E) ≥ m + 2×log(1/ε), then the output is
        ε-close to uniform, independent of Eve's information.

    Args:
        key_bits: Input sifted key as a binary array {0, 1}^n.
        output_length: Desired compressed key length m < n.
        seed: RNG seed for Toeplitz matrix generation.

    Returns:
        Compressed key as binary array {0, 1}^output_length.
    """
    ...
    if output_length >= n:
        ...
        return key_bits[:output_length]

    # Generate Toeplitz matrix from (n + output_length - 1) random bits
    toeplitz_seed = rng.integers(0, 2, size=n + output_length - 1)

    # Construct Toeplitz matrix rows efficiently
    # Row i of T is: toeplitz_seed[output_length - 1 - i : output_length - 1 - i + n]
    compressed = np.zeros(output_length, dtype=int)
    for i in range(output_length):
        row_start = output_length - 1 - i
        row = toeplitz_seed[row_start:row_start + n]
        compressed[i] = int(np.dot(row, key_bits)) % 2

    return compressed


def hoeffding_security_bound(
    n_samples: int,
    delta: float = 0.05,
) -> float:
    """
    Computes the finite-key security parameter using the Hoeffding inequality.

    For n independent samples, the probability that the sample mean deviates
    from the true mean by more than ε is bounded by:
        P(|X̄ - μ| > ε) ≤ 2 × exp(-2nε²)
    ...
    Args:
        n_samples: Number of sifted key bits used for parameter estimation.
        delta: Security parameter (probability of failure). Default 0.05.

    Returns:
        ε — the maximum deviation bound on QBER estimation.
    """
    eps = np.sqrt(np.log(2.0 / delta) / (2.0 * n_samples))
    return float(eps)
